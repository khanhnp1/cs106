...
def check_move(grid, x1, y1, x2, y2):
    ...
    if y2 < len(grid) and x2 < len(grid[0]) and grid[y2][x2] == None:
        if x1 != x2 and grid[y1][x2] != None:
            return False
        else:
            return True
...
def do_gravity(grid, x, y):
    ...
    if grid[y][x] == 's':
        if check_move(grid, x, y, x, y + 1):
            do_move(grid, x, y, x, down_y)
        elif check_move(grid, x, y, down_left_x, down_y):
            do_move(grid, x, y, down_left_x, down_y)
        elif check_move(grid, x, y, down_right_x, down_y):
            do_move(grid, x, y, down_right_x, down_y)
    return grid    
...
def do_brownian(grid, x, y, brownian):
    ...
    if grid[y][x] == 's':
        ...
        if num < brownian:
            ...
            x1 = (x - 1) if (coin == 0) else (x + 1)
            if check_move(grid, x, y, x1, y):
                do_move(grid, x, y, x1, y)
    return grid
