...
def add_data_for_word(word_data, word, gender, rating):
    ...
    if word_data.get(word) == None:
        default_dict[gender][convert_rating_to_index(rating)] = 1
    ...
    with open(filename) as file:
        ...
        for line in file:
            ...
            for word in word_list:
                add_data_for_word(word_data, word, line[1], float(line[0]))
...
def search_words(word_data, target):
    ...
    for key in word_data.keys():
        if key.find(target.upper()) != -1 or key.find(target.lower()) != -1:
            matching_words[key] = word_data[key]
