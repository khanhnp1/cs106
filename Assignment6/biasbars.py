...
def get_centered_x_coordinate(width, idx):
    ...
    """
    actual_width = width - LEFT_MARGIN - RIGHT_MARGIN
    if idx == 0:
        x_coordinate = actual_width / 6 + LEFT_MARGIN
    elif idx == 1:
        x_coordinate = actual_width / 2 + LEFT_MARGIN
    elif idx == 2:
        x_coordinate = 5 * actual_width / 6 + LEFT_MARGIN
...
def draw_fixed_content(canvas):
    ...
    for i in range(3):
        canvas.create_text(round(get_centered_x_coordinate(WINDOW_WIDTH, i), 1), height - VERTICAL_MARGIN + LABEL_OFFSET, text=LABELS[i])
...
def plot_word(canvas, word_data, word):
    ...
    # add your code here
    TICK_STEP = (height - (2 * VERTICAL_MARGIN)) / NUM_VERTICAL_DIVISIONS
    max_height = height - VERTICAL_MARGIN
    for i in range(NUM_VERTICAL_DIVISIONS + 1):
        left_x = LEFT_MARGIN - (TICK_WIDTH) // 2
        right_x = LEFT_MARGIN + (TICK_WIDTH) // 2
        tick_y = VERTICAL_MARGIN + i * TICK_STEP
        tick_val = round(max_frequency - (i * max_frequency / 7))

        canvas.create_line(left_x, tick_y, right_x, tick_y, width=LINE_WIDTH)
        canvas.create_text(left_x - LABEL_OFFSET, tick_y, text=str(tick_val))

    for i in range(3):
        center_x = round(get_centered_x_coordinate(WINDOW_WIDTH, i), 1)
        left_x  = center_x - (BAR_WIDTH)
        right_x = center_x + (BAR_WIDTH)
        top_w_y = max_height - (gender_data[biasbarsdata.KEY_WOMEN][i] / max_frequency) * (max_height - VERTICAL_MARGIN)
        top_m_y = max_height - (gender_data[biasbarsdata.KEY_MEN][i] / max_frequency) * (max_height - VERTICAL_MARGIN)

        canvas.create_rectangle(left_x, top_w_y, center_x, height - VERTICAL_MARGIN, fill='dodgerblue')
        canvas.create_rectangle(center_x, top_m_y, right_x, height - VERTICAL_MARGIN, fill='orange')
        if top_w_y < max_frequency:
            canvas.create_text(left_x + TEXT_DX, top_w_y, text="W", anchor=tkinter.NW)
        if top_m_y < max_frequency:
            canvas.create_text(center_x + TEXT_DX, top_m_y, text="M", anchor=tkinter.NW)
